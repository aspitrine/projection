...
- [ ] Catalog Bun : `effect`, `@effect/sql-pg`, `@effect/atom-react`, `@effect/vitest`, `@effect/platform-node` épinglés sur la même rc.
- [ ] Retirer oRPC, Drizzle, Zod, TanStack Query (`packages/api`, `packages/db` supprimés).
- [ ] `@projection/shared-kernel` : IDs brandés, `CurrentActor`, erreurs transverses.
- [ ] `@projection/platform` : Layer `PgClient` depuis `Config`, runner de migrations par contexte, Layer d'observabilité.
- [ ] Composition root `apps/web/src/server` : `HttpRouter.toWebHandler` monté sur la route TanStack `/api/rpc`, RPC `health`.
- [ ] Client : `AtomRpc` + `@effect/atom-react`, appel `health` affiché sur la page d'accueil (SSR + client).
- [ ] `@effect/vitest` configuré, un test par couche en exemple.
- [ ] Gabarit de contexte documenté (`domain` / `application` / `infrastructure` / `api` / `migrations`, exports `contract` / `domain` / `server`).
