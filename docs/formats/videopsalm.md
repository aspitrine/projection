...
Archive **ZIP** (entrées non compressées, méthode *Stored*). Contenu :
...
| `SongBook_N.json` | Recueil d'origine de l'élément *N* : `Abbreviation`, `VersionDate` (`yyyyMMddHHmmss`), `Guid`, `Text` (nom), `Description`, `Songs: []` |
| `Song_N.json` | Chant de l'élément *N* (voir ci-dessous) |
...
L'ordre des éléments de l'agenda est l'index *N*.
